...
# This is not my code, nor do I claim it to be. I am simply using this as skeleton / template code
# to build a paper trading bot containing my specifications.
...
class Trader:
    ...
    def is_tradable(self,ticker,direction=False):
        ...
        try:
            ...
            self._L.info('Asset %s status is unresponsive.' % ticker)
            pass

        self._L.info('%s is NOT tradable at this time.' % ticker)
        return False
    ...
    def set_stoploss(self,stopLoss,direction):
        ...
        try:
            ...
            self._L.info('STOP LOSS ERROR! Instruction was not clear when setting the stop loss.')
            self._L.info(str(e))
            self.stopLoss = float(stopLoss)

        self._L.info('Stop loss set at %.2f' % self.stopLoss)
    ...
    def set_takeprofit(self,entryPrice,stopLoss):
        ...
        try:
            ...
            self._L.info('CASHING PROFIT ERROR! Instruction was not clear when setting stop loss!')
            self._L.info(e)
    ...
    def get_shares_from_equity(self,assetPrice):
        ...
        if float(account.buying_power) < self.operEquity:
            self._L.info('Insufficient buying power (%d$), aborting process' % float(account.buying_power))
            time.sleep(3)
            return False
        else:
    ...
    def load_historical_data(self,stock,interval='1Min',limit=100):
        ...
        while True:
            try: # fetch the data
                ...
                self._L.info('WARNING: Could not load historical data, retrying')
                self._L.info(e)
            ...
            try: # check if the data is updated
                ...
                if diff <= timedeltaItv:
                    ...
                    if gvars.maxAttempts['LHD1'] >= attempt >= gvars.maxAttempts['LHD2']:
                        self._L.info('Fetching data, process is currently retrying (%d)...' % attempt)
                        ...
                        self._L.info('WARNING: Max attempts reached: (%d), trying to fetch data, retrying...' % attempt)
                ...
                self._L.info('ERROR: Unable to verify if data has been updated')
                self._L.info(str(e))
                time.sleep(gvars.sleepTimes['LH'])
    ...
    def submitOrder(self,orderDict):
        ...
        while attempt < gvars.maxAttempts['SO']:
            try:
                ...
                self._L.info('WARNING: order of | %d %s %s | failed.' % (qty,symbol,side))
                self._L.info(str(e))
                time.sleep(gvars.sleepTimes['SO'])
                attempt += 1

        self._L.info('WARNING: Unable to submit the order, aborting process.')
        return False
    ...
    def cancelOrder(self,orderId):
        ...
        while attempt < gvars.maxAttempts['CO']:
            try:
                ...
                self._L.info('WARNING: Failed to cancel order, trying again.')
                self._L.info(e)
    ...
    def get_stochastic(self,stock,direction,loadHist=False):
        ...
        try:
            ...
            self._L.info('ERROR: failed to retrieve stochastics data.')
    ...
    def enter_position_mode(self,stock,desiredPrice,sharesQty):
        ...
        while True:
            ...
            if not self.check_position(stock):
                self._L.info('Warning: Position unavailable on Alpaca trading platform.')
                return False
            else:
            ...
            if (
                ...
                self._L.info('STOP LOSS reached at price %.3f' % currentPrice)
                self.success = 'NO: STOP LOSS'
                break # break the while loop

            # if take profit reached
            elif currentGain >= targetGain:
                self._L.info('# Target gain reached at %.3f. BYE   #' % currentPrice)
                self.success = 'YES: TARGET GAIN'
                break # break the while loop

            # if stochastics crossed otherwise
            elif stochCrossed:
                self.success = 'YES: STOCHASTIC CROSSED WITH GAIN'
                break # break the while loop
    ...
    def run(self,stock):
        ...
        if self.check_position(stock,maxAttempts=2): # check if the position exists beforehand
            self._L.info('There is already a position open FOR STOCK %s, aborting.' % stock.name)
        ...
        while True:
            ...
            if not self.check_position(stock): # check if the order has EXISTS
                self._L.info('Order did not register to create a position, cancelling order.')
                self.cancelOrder(self.order.id)
                self._L.info('Order cancelled successfully.')
                return stock.name,False

            try: # go on and enter the position
                self.enter_position_mode(stock,currentPrice,sharesQty)
            except Exception as e:
                self._L.info('ERROR: error when entering position.')
                self._L.info(str(e))
                block_thread(self._L,e,self.thName,stock.name)
            ...
            try:
                if 'YES' in self.success:
                    ...
                    self._L.info('Blocking asset due to poor strategy.')
                    return stock.name,True
            except Exception as e:
                self._L.info('ERROR: Failed to successfully process.')
                self._L.info(str(e))
